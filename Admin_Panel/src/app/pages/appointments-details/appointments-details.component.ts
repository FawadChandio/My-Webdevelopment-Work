  ...
  getData() {
    ...
    this.api.post_private('v1/appoinments/getInfoAdmin', { id: this.id }).then((data: any) => {
      ...
      if (data && data.status && data.status == 200 && data.data && data.data.id) {
        ...
        if (info.specialistInfo && info.specialistInfo.first_name && info.specialistInfo.id == info.specialist_id) {
          ...
          console.log(`Name --> ${this.specialistName} ${this.specialistCover}`);
        }
