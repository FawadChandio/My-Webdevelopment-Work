  ...
  getInfo() {
    ...
    this.api.post_private('v1/appoinments/getInfo', { id: this.id }).then((data: any) => {
      ...
      if (data && data.status && data.status == 200 && data.data && data.data.id) {
        ...
        if (info && info.specialistInfo && info.specialistInfo.first_name) {
          this.specialistName = `${info.specialistInfo.first_name} ${info.specialistInfo.last_name}`;
          this.specialistCover = info.specialistInfo.cover;
        }
        console.log(`Notes ==> ${this.orderNotes}`);
        console.log(`Specialist Name --> ${this.specialistName}`);
        console.log(`Specialist Cover --> ${this.specialistCover}`);
