...
export class CheckoutComponent implements OnInit {
  ...
  @ViewChild('id2') addressCollapse: CollapseComponent;
  @ViewChild('id3') slotCollapse: CollapseComponent;
  @ViewChild('id4') specialistCollapse: CollapseComponent;
  @ViewChild('idservice') idserviceCollapse: CollapseComponent;
  ...
  saveSpecialist(id: any, name: String) {
    this.selectedSpecialist = id;
    this.specialistName = name;
    this.specialistCollapse.toggle();
  }
  ...
  getSpecialist() {
    ...
    this.api.post_private('v1/specialist/getSpecialist', { "id": uid }).then((data: any) => {
  ...
  getFreelancerByID() {
    ...
    this.api.post('v1/salon/salonDetails', param).then((data: any) => {
      if (data && data.status == 200) {
        console.log(data);
  ...
  selectedOffers(item: any) {
    ...
    if (ids.includes(this.storeInfo.uid)) {
      ...
      this.util.errorMessage(this.util.translate('This coupon is not valid for ') + this.storeInfo.user_info.first_name);
    }
  }
  ...
  onCheckout() {
    ...
    console.log(this.serviceAt);
    if (this.serviceAt == 'home') {
  ...
  stripeWebCheckout() {
    console.log('Stripe Web Checkout');
    let uid = '';
    if (this.serviceCart.serviceCart.length > 0) {
      uid = this.serviceCart.serviceCart[0].uid;
    } else if (this.serviceCart.packagesCart.length > 0) {
    ...
    const param = {
      ...
      "appointments_to": this.serviceAt == 'home' ? 1 : 0,
      "address": this.serviceAt == 'home' ? JSON.stringify(this.serviceCart.deliveredAddress) : 'NA',
    ...
    this.api.post_private('v1/appoinments/create', param).then((data: any) => {
      ...
      if (data && data.status && data.status == 200 && data.data && data.data.id) {
        ...
        const grandTotal = convertToCents(this.serviceCart.grandTotal);
        ...
        this.util.start();
        this.api.post_private('v1/payments/stripeWebCheckout', stripeParam).then((data: any) => {
          console.log('stripe pay response', data);
          this.util.stop();
          if (data && data.status && data.status == 200 && data.success && data.success.url != '') {
            const navParma: NavigationExtras = {
              queryParams: {
                id: orderId,
                payLink: data.success.url
              }
            }
